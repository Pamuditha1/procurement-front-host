...
function SEViewMSR({ viewItems }) {
  ...
  const viewModal = (items, id, status, reasons, p) => {
    viewItems(items, id, status, reasons, p);
    // console.log("clicked");
    // setmodalItems(items);
    // setisModalOpen(true);
  };
          ...
          {msrs.map((p) => {
            ...
            return (
              <tr style={{ textAlign: "center" }} key={p._id}>
                <td>
                  <Link to="/site-engineer/view-msr/items">
                    <button
                      onClick={() =>
                        viewModal(p.items, p._id, p.status, p.reasons, p)
                      }
